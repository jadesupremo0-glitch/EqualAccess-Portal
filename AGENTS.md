...
- `src/pages/admin/Recapitulation.tsx` + `src/components/recapitulation/` + `src/lib/recapitulation/` — admin-only **PWD Recapitulation** (per-barangay counts by age bracket + DOH PRPWD encoding status, kept as dated snapshots; CSV/Excel export, print, optional read-only landing section). Data lives in `recapitulation_*` tables (migration `20260922010000_recapitulation.sql`). Tables have no write policies: everything goes through `get_recap_reports` / `save_recap_report` / `delete_recap_report`, which re-check the admin's id + password and save in one transaction; totals and percentages are generated columns. Exports are built in the browser (`export.ts`, ExcelJS loaded on demand). Seed the April 30, 2026 snapshot with `npm run seed:recap`.
