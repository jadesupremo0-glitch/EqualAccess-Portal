...
// Attempt to read optional Figma site configuration. Some environments
// (local dev) may not have `./.figma/make/site.json`, so fall back to {}
let siteConfiguration: any = {}
try {
  const sitePath = path.resolve(__dirname, './.figma/make/site.json')
  if (fs.existsSync(sitePath)) {
    siteConfiguration = JSON.parse(fs.readFileSync(sitePath, 'utf8'))
  }
} catch (err) {
  siteConfiguration = {}
}
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "EqualAccess Portal"
